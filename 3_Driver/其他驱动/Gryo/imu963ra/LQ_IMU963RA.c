...
static void imu963ra_write_acc_gyro_register(uint8 reg, uint8 data)
{
	...
	SPI_ReadWriteNByte(IMU963RA_SPI,0,dat,dat,2);	//用的CS0片选0引脚
}
...
static void imu963ra_read_acc_gyro_registers(uint8 *data, uint32 len)
{
	...
	SPI_ReadWriteNByte(IMU963RA_SPI,0,data,data,len);	//用的CS0片选0引脚
}
#endif
...
uint8 imu963ra_init (void)
{
	...
	LQ_SPI_Master_Init(IMU963RA_SPI, 0, 3, 1*1000*1000);//cs0，mode3
#endif
    ...
    imu963ra_write_acc_gyro_register(IMU963RA_INT1_CTRL, 0x03);         // 开启加速度陀螺仪数据就绪中断
