...
//  {
//      WitSerialDataIn(USART2_RX_BUF[0]);
//      UART_Start_Receive_IT(huart, USART2_RX_BUF, 1); // 收到一个数据后进入中断
//  }
//}


// unsigned long uiBuad = 115200;
// unsigned char ucRxData = 0;
//使用void Gryo_init(void)    void Gryo_get(void)
float fAcc[3], fGyro[3], fAngle[3], fYaw;   
static char s_cDataUpdate = 0;//选择更新哪个
...
static void AutoScanSensor(void);//自动搜索传感器
static void SensorUartSend(uint8_t *p_data, uint32_t uiSize);// 发送串口数据
static void CopeSensorData(uint32_t uiReg, uint32_t uiRegNum);// 处理传感器数据

// 陀螺仪初始化
void Gryo_init(void)
{
	WitInit(WIT_PROTOCOL_NORMAL, 0x50);//初始化标准协议，设置设备地址
	WitSerialWriteRegister(SensorUartSend);//注册串口发送函数
	WitRegisterCallBack(CopeSensorData); //注册获取传感器数据回调函数
	AutoScanSensor();//自动搜索传感器
}

// 陀螺仪接收数据   （中断：100hz）
void Gryo_Update(void) // 测量串口数据
{
	u8 i;
	// HAL_Delay(500);   //避免刷新太快观察不清楚
	if(s_cDataUpdate) // 检测数据更新的标志位
	{
		//printf("3");
		for(i = 0; i < 3; i++)  // 处理数据
		{
...
{
	u8 i;
	//HAL_Delay(100);   //避免刷新太快观察不清楚
	if(s_cDataUpdate)//接收到了数据的标志位
	{
		for(i = 0; i < 3; i++)
		{
			fAcc[i] = sReg[AX+i] / 32768.0f * 16.0f;
...
//CmdProcess();//校准
//int32_t WitStartAccCali(void);//校准Acc
...
//处理串口接收的数据，由WitSerialDataIn(USART2_RX_BUF[0]);调用
...
static void AutoScanSensor(void)
{
	...
	{
        uiBuad = c_uiBaud[i];
//		huart2.Init.BaudRate = uiBuad;
//		HAL_UART_Init(&huart2);  // 重新初始化
		iRetry = 2;
		do
		{
			s_cDataUpdate = 0;
